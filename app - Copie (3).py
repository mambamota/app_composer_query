    ...
    if not old_schema or not new_schema:
        ...
        if st.session_state['table_map'] and st.session_state['column_maps'] and st.session_state['relevant_old_tables']:
            ...
            if st.button("Transform Query"):
                new_query = transform_query_full(st.session_state['old_query'], st.session_state['table_map'], st.session_state['column_maps'])
                ...
                <table class="diff-table">
                    <tr>
                        <th>Old #</th><th>Old Query</th><th>New #</th><th>New Query</th>
                    </tr>
                '''
                for i in range(max_lines):
                    old_line = old_lines[i] if i < len(old_lines) else ''
                    new_line = new_lines[i] if i < len(new_lines) else ''
                    html += f'<tr><td align="right">{i+1 if i < len(old_lines) else ""}</td><td>{old_line}</td><td align="right">{i+1 if i < len(new_lines) else ""}</td><td>{new_line}</td></tr>'
                html += '</table>'
                ...
                if summary_rows:
                    ...
                    <table class="diff-table">
                        <tr>
                            <th>Old Line</th><th>New Line</th><th>Old Content</th><th>New Content</th>
                        </tr>'''
